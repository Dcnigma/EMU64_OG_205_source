...
//					if(dspIRQClear)
			...
			case 0x88:
				{
#ifdef KI2
					//for KI2, this is DipSwitch port
...
//						*(DWORD *)&m->aiReg[0x90]=inputs[0]&0xffff;
#endif
//						*(WORD *)&m->aiReg[0x88]=inputs[2];
/*
*(88086244)
						BYTE *patch=(BYTE *)(0x8F5F7+m->rdRam);
						if(theApp.m_IgnoreSecondary)
							*patch=0;
						else
							*patch=1;
						BYTE *patch=(BYTE *)(0x86244+m->rdRam);
						*patch=1;
						patch=(BYTE *)(0x8F5E2+m->rdRam);
						*patch=1;
						patch=(BYTE *)(0x8622B+m->rdRam);
						*patch=1;
*/
...
#ifdef LOG_STUFF
//						if((cheat!=0xaa)&&(cheat!=0x55))
						theApp.LogMessage("B-Range Write %X to %X, PC %X",cheat,VAddr&0xffff,r->PC);
//						return(m->NullMem);
						theApp.LogMessage("0x5f8 %X",*(WORD *)&m->dspDMem[0x5f8<<1]);
						theApp.LogMessage("0x5f9 %X",*(WORD *)&m->dspDMem[0x5f9<<1]);
						theApp.LogMessage("0x5fa %X",*(WORD *)&m->dspDMem[0x5fa<<1]);
						theApp.LogMessage("0x5fb %X",*(WORD *)&m->dspDMem[0x5fb<<1]);
						theApp.LogMessage("0x5fc %X",*(WORD *)&m->dspDMem[0x5fc<<1]);
						theApp.LogMessage("0x5fd %X",*(WORD *)&m->dspDMem[0x5fd<<1]);
						theApp.LogMessage("0x5fe %X",*(WORD *)&m->dspDMem[0x5fe<<1]);
						theApp.LogMessage("0x5ff %X",*(WORD *)&m->dspDMem[0x5ff<<1]);
						theApp.LogMessage("0x60b %X",*(WORD *)&m->dspDMem[0x60b<<1]);
						theApp.LogMessage("0x60d %X",*(WORD *)&m->dspDMem[0x60d<<1]);
#endif
						break;
					}
				case 0x100:
...
void iMemCopyBootCode()
{
	...
	CFile *arom=(CFile *)new CFile();
	arom->Open(theApp.m_ARom1,CFile::modeRead);
	arom->Read(dst,512*1024);
	arom->Close();
	delete arom;
	dst+=512*256;
	
	arom=(CFile *)new CFile();
	arom->Open(theApp.m_ARom2,CFile::modeRead);
	arom->Read(dst,512*1024);
	arom->Close();
	delete arom;
	dst+=512*256;
	...
	arom=(CFile *)new CFile();
	arom->Open(theApp.m_ARom3,CFile::modeRead);
	arom->Read(dst,512*1024);
	arom->Close();
	delete arom;
	dst+=512*256;
	...
	arom=(CFile *)new CFile();
	arom->Open(theApp.m_ARom4,CFile::modeRead);
	arom->Read(dst,512*1024);
	arom->Close();
	delete arom;
	dst+=512*256;
	...
	arom=(CFile *)new CFile();
	arom->Open(theApp.m_ARom5,CFile::modeRead);
	arom->Read(dst,512*1024);
	arom->Close();
	delete arom;
	dst+=512*256;
	...
	arom=(CFile *)new CFile();
	arom->Open(theApp.m_ARom6,CFile::modeRead);
	arom->Read(dst,512*1024);
	arom->Close();
	delete arom;
	...
	dst+=512*256;
	arom=(CFile *)new CFile();
	arom->Open(theApp.m_ARom7,CFile::modeRead);
	arom->Read(dst,512*1024);
	arom->Close();
	delete arom;
	...
	dst+=512*256;
	arom=(CFile *)new CFile();
	arom->Open(theApp.m_ARom8,CFile::modeRead);
	arom->Read(dst,512*1024);
	arom->Close();
	delete arom;
}
...
void iMemLoadShort(CFile *tmp)
{
	tmp->Read(iMemAddr[0],65536);
	tmp->Read(iMemAddr[1],MemSize[1]);
	tmp->Read(iMemAddr[2],MemSize[2]);
	tmp->Read(iMemAddr[4],MemSize[4]);
	for(int i=7;i<17;i++)
	{
		tmp->Read(iMemAddr[i],MemSize[i]);
	}
...
unsigned _int16 dspReadWord(DWORD A)
{
	WORD tmp=0;
	A>>=1;
/*
